...
- Primary UX should focus on a single managed tmux session: `clawmaster-host-terminal`.
...
Phase-1 constraint:
- `session` is fixed to `clawmaster-host-terminal`.
...
- `GET /api/terminal/windows` (for `clawmaster-host-terminal`)
...
- top tab strip with:
  - active tmux-window tabs (same `clawmaster-host-terminal` session),
...
## UX behavior
- First load:
  - if tmux available: open default tab for `clawmaster-host-terminal`.
...
- Lock to `clawmaster-host-terminal` and windows-only tabs.
