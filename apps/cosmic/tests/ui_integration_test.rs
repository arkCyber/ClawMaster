...
mod ui_integration_tests {
    ...
    fn test_menu_system_completeness() {
        use clawmaster_cosmic::widgets::{
            edit_menu_items, file_menu_items, help_menu_items, view_menu_items,
        };
        ...
        assert!(
            file_items.len() >= 5,
            "File menu should have at least 5 items"
        );
        assert!(
            file_items.iter().any(|i| i.label.contains("New Session")),
            "Should have New Session"
        );
        assert!(
            file_items.iter().any(|i| i.label.contains("Quit")),
            "Should have Quit"
        );
        ...
        assert!(
            edit_items.len() >= 6,
            "Edit menu should have at least 6 items"
        );
        assert!(
            edit_items.iter().any(|i| i.label.contains("Copy")),
            "Should have Copy"
        );
        assert!(
            edit_items.iter().any(|i| i.label.contains("Paste")),
            "Should have Paste"
        );
        ...
        assert!(
            view_items.len() >= 5,
            "View menu should have at least 5 items"
        );
        assert!(
            view_items.iter().any(|i| i.label.contains("Dashboard")),
            "Should have Dashboard"
        );
        assert!(
            view_items.iter().any(|i| i.label.contains("Chat")),
            "Should have Chat"
        );
        ...
        assert!(
            help_items.len() >= 4,
            "Help menu should have at least 4 items"
        );
        assert!(
            help_items.iter().any(|i| i.label.contains("About")),
            "Should have About"
        );
    }
    ...
    fn test_menu_shortcuts() {
        ...
        assert_eq!(
            new_session.unwrap().shortcut.as_deref(),
            Some("⌘N"),
            "New Session shortcut should be ⌘N"
        );

        let quit = file_items.iter().find(|i| i.label.contains("Quit"));
        assert!(quit.is_some(), "Quit menu item should exist");
        assert_eq!(
            quit.unwrap().shortcut.as_deref(),
            Some("⌘Q"),
            "Quit shortcut should be ⌘Q"
        );
    }
    ...
    fn test_menu_item_states() {
        ...
        // 新建会话应该是启用的
        let new_session = file_items
            .iter()
            .find(|i| i.label.contains("New Session"))
            .unwrap();
        assert!(new_session.enabled, "New Session should be enabled");

        // Open Project 现在是启用的
        let open_project = file_items
            .iter()
            .find(|i| i.label.contains("Open Project"))
            .unwrap();
    ...
    fn test_page_navigation_coverage() {
        use {clawmaster_cosmic::app_new::Page, std::mem::discriminant};
        ...
        for page in &pages {
            assert!(
                unique_pages.insert(discriminant(page)),
    ...
    fn test_window_configuration() {
        ...
        assert!(
            min_width >= 1200.0,
            "Minimum width should be at least 1200px"
        );
        assert!(
            min_height >= 700.0,
            "Minimum height should be at least 700px"
        );
        assert!(
            max_width <= 2000.0,
            "Maximum width should not exceed 2000px"
        );
        assert!(
            max_height <= 1400.0,
            "Maximum height should not exceed 1400px"
        );
    }
}
