...
use clawmaster_cosmic::pages::{
    agents::create_mock_agents, heartbeat::create_mock_heartbeat, identity::create_mock_identity,
...
mod identity_tests {
    ...
    fn test_identity_config_creation() {
        ...
        assert!(
            !config.ui_language.is_empty(),
            "UI language should not be empty"
        );
    }
...
mod agents_tests {
    ...
    fn test_agents_creation() {
        ...
        assert!(
            agents.iter().any(|a| a.enabled),
            "At least one agent should be enabled"
        );
    }
...
mod providers_tests {
    ...
    fn test_providers_creation() {
        ...
        assert!(
            ...
            "Anthropic provider should exist"
        );
    }
    ...
    fn test_provider_states() {
        ...
        // 验证有启用和禁用的提供商
        assert!(
            providers.iter().any(|p| p.enabled),
            "At least one enabled provider"
        );
        assert!(
            providers.iter().any(|p| !p.enabled),
            "At least one disabled provider"
        );
    }
}
...
mod heartbeat_tests {
    ...
    fn test_heartbeat_creation() {
        ...
        // 验证关键组件存在
        assert!(
            heartbeats.iter().any(|h| h.component.contains("Gateway")),
            "Gateway component"
        );
        assert!(
            heartbeats.iter().any(|h| h.component.contains("Database")),
            "Database component"
        );
    }
    ...
    fn test_performance_metrics() {
        ...
        for hb in &heartbeats {
            ...
            // 验证正常运行时间百分比
            assert!(
                hb.uptime_percent >= 0.0 && hb.uptime_percent <= 100.0,
