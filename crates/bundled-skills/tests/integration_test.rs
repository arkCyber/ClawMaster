...
use clawmaster_bundled_skills::{all_bundled_skills, get_skills_by_category, install_bundled_skills};
...
fn test_all_skills_have_metadata() {
    ...
    for skill in &skills {
        ...
        assert!(skill.body.contains("---"), "Skill body must contain frontmatter");
        assert!(skill.body.contains(&format!("name: {}", skill.metadata.name)), 
...
fn test_category_productivity() {
    ...
    assert_eq!(skills.len(), 6, "Productivity category should have 6 skills");
...
async fn test_install_bundled_skills() {
    ...
    for skill in &skills {
        ...
        assert!(skill_dir.exists(), "Skill directory should exist: {:?}", skill_dir);
...
async fn test_install_creates_directories() {
    ...
    install_bundled_skills(install_dir).await.expect("Installation failed");
...
fn test_skill_names_are_valid() {
    ...
    for skill in &skills {
        ...
        assert!(!name.starts_with('-'), "Name cannot start with hyphen: {}", name);
        assert!(!name.ends_with('-'), "Name cannot end with hyphen: {}", name);
        
        // No double hyphens
        assert!(!name.contains("--"), "Name cannot contain double hyphens: {}", name);
    }
}
