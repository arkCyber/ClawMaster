...
impl AIAssistant {
    ...
    fn find_skills_for_intent(&self, user_request: &str) -> Vec<String> {
        ...
        } else if request_lower.contains("code")
            || request_lower.contains("coding")
        ...
        } else if request_lower.contains("calendar")
            || request_lower.contains("日历")
            || request_lower.contains("schedule")
            || request_lower.contains("meeting")
        {
...
fn test_conversation_note_taking() {
    ...
    assert!(
        !recommended_skills.is_empty(),
        "AI should find note-taking skills"
    );
...
fn test_conversation_email_check() {
    ...
    assert!(
        recommended_skills.contains(&"himalaya".to_string())
...
fn test_conversation_music_playback() {
    ...
    assert!(!recommended_skills.is_empty());
    assert!(
        recommended_skills.contains(&"spotify".to_string())
...
fn test_conversation_task_management() {
    ...
    assert!(!recommended_skills.is_empty());
    assert!(
        recommended_skills.contains(&"things-mac".to_string())
...
fn test_conversation_skill_availability() {
    ...
    for skill in common_skills {
        assert!(
            ai.has_skill(skill),
            "Common skill {} should be available",
            skill
        );
...
fn test_conversation_calendar_management() {
    ...
    assert!(!recommended_skills.is_empty());
    assert!(
        recommended_skills.contains(&"calendar".to_string())
            || recommended_skills.contains(&"gog".to_string())
    );
}
...
fn test_conversation_code_assistance() {
    ...
    assert!(
        recommended_skills.contains(&"github".to_string())
...
fn test_conversation_all_categories_accessible() {
    ...
    for category in expected_categories {
        assert!(
            ai.skill_categories.contains_key(category),
...
fn test_conversation_skill_metadata_complete() {
    ...
    for skill_name in &ai.available_skills {
        ...
        assert!(info.is_some(), "Skill {} should have metadata", skill_name);

        let description = info.unwrap();
        assert!(
            !description.is_empty(),
            "Skill {} should have non-empty description",
            skill_name
        );
    }
}
...
fn test_conversation_real_world_workflow() {
    ...
    for (request, expected_skills) in workflow {
        ...
        let has_expected = expected_skills
            .iter()
