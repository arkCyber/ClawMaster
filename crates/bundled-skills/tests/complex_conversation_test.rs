...
impl AdvancedAIAssistant {
    ...
    fn process_question(&mut self, question: &str) -> ConversationTurn {
        ...
        let skill_descriptions: Vec<String> = skills.iter()
            .map(|skill_name| {
                let desc = self.available_skills.get(skill_name)
                    .cloned()
    ...
    fn analyze_intent(&self, question: &str) -> (String, String, Vec<String>, String, f32, Vec<String>) {
        // Notes intent
        ...
        if question.contains("email") || question.contains("邮件") || question.contains("mail") {
            return (
                ...
                "识别邮件需求，himalaya 提供 IMAP/SMTP 支持，gog 提供 Google Workspace 集成".to_string(),
                0.90,
        ...
        // GitHub/Code intent
        if question.contains("github") || question.contains("代码") || 
           question.contains("code") || question.contains("issue") ||
           question.contains("pull request") || question.contains("pr") {
            return (
                ...
                0.98,
                vec!["可以使用 git 命令行".to_string(), "可以使用 GitHub CLI".to_string()],
            );
        }
        ...
        // Music intent
        if question.contains("music") || question.contains("音乐") || 
           question.contains("play") || question.contains("song") {
            return (
                "音乐播放".to_string(),
        ...
        // Messaging intent
        if question.contains("message") || question.contains("消息") || 
           question.contains("slack") || question.contains("discord") ||
        ...
        if question.contains("calendar") || question.contains("日历") ||
           question.contains("schedule") || question.contains("meeting") ||
           question.contains("appointment") || question.contains("会议") {
            return (
                "日程管理".to_string(),
        ...
        if question.contains("task") || question.contains("任务") ||
           question.contains("todo") || question.contains("待办") {
            return (
                "任务管理".to_string(),
                "用户需要管理任务或待办事项".to_string(),
                vec!["things-mac".to_string(), "trello".to_string(), "apple-reminders".to_string()],
        ...
        (
            ...
            0.0,
            vec!["请提供更多细节".to_string(), "可以浏览所有可用 Skills".to_string()],
        )
    }
...
"#,
            ...
            self.conversation_history.iter()
                .map(|t| t.confidence)
                .sum::<f32>() / self.conversation_history.len() as f32 * 100.0,
            self.conversation_history.iter()
...
fn test_complex_productivity_suite() {
    ...
    for turn in &ai.conversation_history {
        assert!(!turn.recommended_skills.is_empty(), 
            "Turn {} should have skill recommendations", turn.turn_number);
    }
}
