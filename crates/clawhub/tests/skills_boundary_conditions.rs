...
use clawmaster_clawhub::types::{SkillMetadata, SkillFormat, SkillSearchQuery, SecurityStatus};
...
async fn test_boundary_special_characters_in_query() {
    ...
    for special in special_queries {
        ...
        let result = skills.search(query).await;
        assert!(result.is_ok(), "Special character '{}' should be handled", special);
    }
}
...
async fn test_boundary_maximum_concurrent_operations() {
    ...
    for _ in 0..1000 {
        ...
        let handle = tokio::spawn(async move {
            skills_clone.increment_downloads("max-concurrent", "1.0.0").await
        });
        handles.push(handle);
    }
    ...
    assert_eq!(retrieved.downloads, 1000, "All concurrent operations must complete");
}
...
async fn test_boundary_all_security_statuses() {
    ...
    for (i, status) in statuses.iter().enumerate() {
        ...
        let result = skills.publish(metadata).await;
        assert!(result.is_ok(), "Security status {:?} should be valid", status);
    }
}
