    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::ApiKeyMissing { provider, help_url } => {
                writeln!(f, "{}", format!("❌ API key for {} is missing", provider).red().bold())?;
                writeln!(f, "   {} Set your API key in one of these ways:", "💡".yellow())?;
                writeln!(f, "      1. Environment variable: {}={}", 
                    format!("{}_API_KEY", provider.to_uppercase()).green(),
                    "sk-your-key".dimmed())?;
                writeln!(f, "      2. Run: {}", 
                    format!("clawmaster provider add {} --key YOUR_KEY", provider.to_lowercase()).green())?;
                writeln!(f, "   {} Get your API key at: {}", "🔗".blue(), help_url.cyan())?;
            }
            
            Self::PermissionDenied { resource, required_permission } => {
                writeln!(f, "{}", "❌ Permission denied".red().bold())?;
                writeln!(f, "   {}: {}", "Resource".dimmed(), resource)?;
                writeln!(f, "   {}: {}", "Required permission".dimmed(), required_permission)?;
                writeln!(f, "   {} Check your access rights or contact an administrator", "💡".yellow())?;
            }
            
            Self::PortInUse { port, suggestion } => {
                writeln!(f, "{}", format!("❌ Port {} is already in use", port).red().bold())?;
            ...
            Self::DatabaseError { operation, suggestion } => {
                writeln!(f, "{}", format!("❌ Database error during: {}", operation).red().bold())?;
                writeln!(f, "   {} {}", "💡".yellow(), suggestion)?;
                writeln!(f, "   {} Try these recovery steps:", "🔧".blue())?;
                writeln!(f, "      1. {}", "clawmaster db status".green())?;
                writeln!(f, "      2. {}", "clawmaster db migrate".green())?;
                writeln!(f, "      3. {}", "clawmaster backup restore <backup-file>".green())?;
            }
            
            Self::ProviderUnavailable { provider, reason, alternatives } => {
                writeln!(f, "{}", format!("❌ Provider '{}' is unavailable", provider).red().bold())?;
            ...
            Self::ChannelError { channel, error, fix_steps } => {
                writeln!(f, "{}", format!("❌ Channel '{}' error", channel).red().bold())?;
            ...
            Self::InvalidConfiguration { field, expected, got } => {
                ...
                writeln!(f, "   {} Run {} to fix your configuration", 
                    "💡".yellow(), 
            ...
            Self::DependencyMissing { dependency, install_command } => {
                writeln!(f, "{}", format!("❌ Missing dependency: {}", dependency).red().bold())?;
    ...
    if err_str.contains("Address already in use") || err_str.contains("port") {
        if let Some(port) = extract_port(&err_str) {
            return UserError::PortInUse {
                port,
                suggestion: format!("Change the port in your config or stop the conflicting service"),
            }.to_string();
        }
    }
