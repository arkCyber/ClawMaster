...
pub async fn execute(component_type: &str, name: &str) -> Result<()> {
    ...
    if !utils::is_valid_name(name) {
        anyhow::bail!("Invalid component name. Use only alphanumeric characters, hyphens, and underscores.");
    }
    ...
    match component_type {
        ...
        _ => anyhow::bail!("Unknown component type: {}. Use 'skill', 'tool', or 'plugin'.", component_type),
    }
...
async fn create_plugin(name: &str) -> Result<()> {
    ...
    println!("  {} Created plugins/{}/plugin.toml", "✓".bright_green(), name);

    let lib_rs = templates::plugin::generate_lib_rs(name);
    fs::write(plugin_path.join("src/lib.rs"), lib_rs)?;
    println!("  {} Created plugins/{}/src/lib.rs", "✓".bright_green(), name);
