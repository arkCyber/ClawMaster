...
use clawmaster_cosmic_client::{CosmicConfig, RpcClient};
use anyhow::Result;
...
async fn test_config_validation_zero_messages_per_page() {
    ...
    let result = config.validate();
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Messages per page"));
}
...
fn test_system_status_health_check() {
    ...
    use clawmaster_cosmic_client::{SystemStatus, ConnectionStatus, MemoryUsage};
