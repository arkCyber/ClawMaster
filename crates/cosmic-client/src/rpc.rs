...
impl RpcClient {
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        let typed_result: T = serde_json::from_value(result)
            .context("Failed to deserialize RPC result")?;
    ...
    pub async fn connect_websocket(&self) -> Result<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>> {
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        Ok(response["version"].as_str().unwrap_or("unknown").to_string())
    }
}
