...
#[async_trait]
impl clawmaster_agents::tool_registry::AgentTool for LocationTool {
    fn name(&self) -> &str {
        "get_user_location"
    }
...
#[cfg(test)]
mod tests {
    use {super::*, clawmaster_agents::tool_registry::AgentTool};
    ...
    async fn reverse_geocode_with_mock_server() {
        ...
        let resp = client
            .get(&url)
            .header("User-Agent", "clawmaster/0.3-test")
            .send()
            .await
            .unwrap();
