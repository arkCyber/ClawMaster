...
fn test_calibrator_realistic_usage() {
    ...
    let tool_executions = vec![
        ...
        ("web_fetch", vec![2_500_000, 2_800_000, 2_600_000, 2_700_000]),
        ("web_search", vec![3_000_000, 3_200_000, 3_100_000, 3_150_000]),
    ];
    ...
    for (tool, measurements) in &tool_executions {
        ...
        assert!(recommended.is_some(), "Should have recommendation for {}", tool);
...
fn test_calibrator_buffer_percentages() {
    ...
    for buffer in buffers {
        ...
        assert_eq!(recommended, expected, "Buffer {} should give {}", buffer, expected);
    }
}
