...
Loaded from `~/.clawmaster/IDENTITY.md` using YAML frontmatter. Fields:
...
Loaded from `~/.clawmaster/SOUL.md`. When the file is absent or empty, the
...
Loaded from `~/.clawmaster/USER.md` using YAML frontmatter.
...
Resolved by `clawmaster_projects::context::load_context_files()`. The loader walks
...
```
Host: host=clawmaster-devbox | os=macos | arch=aarch64 | shell=zsh | time=2026-02-17 16:18:00 CET | today=2026-02-17 | provider=openai | model=gpt-5 | session=main | sudo_non_interactive=true | timezone=Europe/Paris
Sandbox(exec): enabled=true | mode=all | backend=docker | scope=session | image=clawmaster-sandbox:abc123 | workspace_mount=ro | network=disabled
```
...
`clawmaster_skills::prompt_gen::generate_skills_prompt()`:
...
Optional markdown files from the data directory (`~/.clawmaster/`):
