...
ClawMaster uses [sqlx](https://github.com/launchbadge/sqlx) for database access and its
...
│   └── src/server.rs                  # orchestrates clawmaster.db migrations
└── memory/
    ├── migrations/
...
|-------|----------|--------|----------------|
| `clawmaster-projects` | `clawmaster.db` | `projects` | `20240205100000_init.sql` |
| `clawmaster-sessions` | `clawmaster.db` | `sessions`, `channel_sessions` | `20240205100001_init.sql` |
| `clawmaster-cron` | `clawmaster.db` | `cron_jobs`, `cron_runs` | `20240205100002_init.sql` |
| `clawmaster-gateway` | `clawmaster.db` | `auth_*`, `passkeys`, `api_keys`, `env_variables`, `message_log`, `channels` | `20240205100003_init.sql` |
| `clawmaster-memory` | `memory.db` | `files`, `chunks`, `embedding_cache`, `chunks_fts` | `20240205100004_init.sql` |
...
```rust
// server.rs
clawmaster_projects::run_migrations(&db_pool).await?;   // 1. projects first
clawmaster_sessions::run_migrations(&db_pool).await?;   // 2. sessions (FK → projects)
clawmaster_cron::run_migrations(&db_pool).await?;       // 3. cron (independent)
...
|----------|----------|--------|
| `clawmaster.db` | `~/.clawmaster/clawmaster.db` | projects, sessions, cron, gateway |
| `memory.db` | `~/.clawmaster/memory.db` | memory (separate, managed internally) |
...
```rust
clawmaster_new_feature::run_migrations(&db_pool).await?;
```
...
1. Check file permissions on `~/.clawmaster/`
...
```bash
sqlite3 ~/.clawmaster/clawmaster.db "SELECT version, description, success FROM _sqlx_migrations ORDER BY version"
```
